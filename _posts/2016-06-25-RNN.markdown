...
| U | (100, 10000) | self.U.shape | hidden layer마다 각각의 단어들이 나올수 있는 확률들이 들어있음 |
| V | (10000, 100) | self.V.shape | |
| W | (100, 10000) | self.W.shape | |

### Forward Propagation

$$ s_t = tanh(U_{x_t} + W_{s_{t-1}}) $$<br>
$$ \hat{y}_t = softmax(V_{s_t}) $$
...
RNN의 경우, 일반적인 Neural Network와는 다르게 time steps을 갖고 있기 때문에, Backpropagation through time (BTPP)를 이용합니다.
