...
```bash
# 다음을 실행후, 한글키를 눌러서 어떤 값이 나오는지 확인
$ xev | grep key
...
[Desktop Entry]
Type=Application
Exec=xmodmap /home/anderson/.Xmodmap
Hidden=false
NoDisplay=false
X-GNOME-Autostart-enabled=true
Name=Xmodmap
Comment=Load custom key mappings
```
...
<img src="{{ page.asset_path }}fcitx5-configure.png" class="img-responsive img-rounded img-fluid center" style="border: 2px solid #333333">
...
1. Input Method >> Korean - Hangul 추가
2. Korean - Hangul >> Preferences >> Korean - Korean (101/104-key compatible) 추가

<img src="{{ page.asset_path }}input-method-korean-hangul-2.png" class="img-responsive img-rounded img-fluid center" style="border: 2px solid #333333">

<img src="{{ page.asset_path }}ibus-setup-hangul-setup.png" class="img-responsive img-rounded img-fluid center" style="border: 2px solid #333333">


다음을 실행해서 Hangul 이 나오는지 확인
...
${font sans-serif:bold:size=12}POWER ${hr 2}${font sans-serif:normal:size=10}
CPU Power (PPT): $alignr ${execi 2 sensors | grep 'PPT' | awk '{print $2}'} W
${execgraph "sensors | grep 'PPT' | awk '{print $2}'" 25,000 000000 00ff00}
GPU Power: $alignr ${execi 2 nvidia-smi --query-gpu=power.draw --format=csv,noheader,nounits} W / ${execi 6000 nvidia-smi --query-gpu=power.limit --format=csv,noheader,nounits} W
${execgraph "nvidia-smi --query-gpu=power.draw --format=csv,noheader,nounits" 25,000 000000 ff0000}
...
## 2.7 부팅후 멈춤 현상 - 터미널/크롬 1분동안 안됨

문제의 로그 

```bash
1min 30.172s xdg-desktop-portal.service  ← 문제 발견!
...
$ journalctl --user -u xdg-desktop-portal-gnome.service -b --no-pager
```

해결책

```bash
# GNOME 포털 비활성화 (GTK 포털로 대체)
sudo mv /usr/share/xdg-desktop-portal/portals/gnome.portal \
        /usr/share/xdg-desktop-portal/portals/gnome.portal.bak
...
현재 Docker version 이 너무 높아서 기존 legacy 지원하는 app 에서 문제를 일으킴. 
예를 들어서 Pycharm 에서 Docker 쓰려면 아래와 같은 에러 나옴
...
# 설치된 Docker 버젼 확인 
$ anderson:openclaw (main)>apt list --installed | grep docker
...
# 예를 들어서 5:28.5.2-1~ubuntu.24.04~noble 이걸로 설치 한다면
sudo apt-get update
