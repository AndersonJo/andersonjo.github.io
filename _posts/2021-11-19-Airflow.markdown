...
PROVIDERS="async,postgres,google,amazon,mysql,redis,elasticsearch,ssh,http,apache-hdfs,docker,cncf-kubernetes,jdbc,ocbc,mongo,slack"
pip install "apache-airflow[${PROVIDERS}]==${AIRFLOW_VERSION}" --constraint "${CONSTRAINT_URL}"
{% endhighlight %}


airflow standalone 을 실행시키면, ~/airflow 디렉토리가 생기고, airflow.cfg 도 생성됩니다.
...
로컬 환경에서 그냥 연습용으로는 standalone 도 나쁘지 않습니다. 
