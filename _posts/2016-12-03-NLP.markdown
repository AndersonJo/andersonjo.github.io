...
Mecab 설치는 다음과 같이 합니다.

{% highlight bash %}
wget https://raw.githubusercontent.com/konlpy/konlpy/master/scripts/mecab.sh
{% endhighlight %}

mecab.sh 파일을 다운받으면 `python3` 명령어를 `sudo python3.6`으로 바꿔줍니다. (Python3.6 사용시)
...
user.csv파일 안에는 다음과 같이 입력을 합니다.

{% highlight csv %}
스파크플러그,,,,NNP,인명,F,스파크플러그,*,*,*,*,*
닥터 스트레인지,,,,NNP,인명,F,닥터 스트레인지,*,*,*,*,*
모킹버드,,,,NNP,인명,F,모킹버드,*,*,*,*,*
{% endhighlight %}

compile 해주가 위해서 다음과 같이 합니다.

{% highlight bash %}
export DIC_PATH=/usr/local/lib/mecab/dic/mecab-ko-dic/
